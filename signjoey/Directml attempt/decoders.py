...
class RecurrentDecoder(Decoder):
    ...
    def _check_shapes_input_forward_step(
        ...
        :param hidden:
        """
        prev_embed = prev_embed.to(dml)
        prev_att_vector = prev_att_vector.to(dml)
        encoder_output = encoder_output.to(dml)
        src_mask = src_mask.to(dml)
        hidden = hidden.to(dml)
    ...
    def _check_shapes_input_forward(
        ...
        :param prev_att_vector:
        """
        trg_embed = trg_embed.to(dml)
        encoder_output = encoder_output.to(dml)
        encoder_hidden = encoder_hidden.to(dml)
        src_mask = src_mask.to(dml)
        hidden = prev_att_vector.to(dml)
        prev_att_vector = prev_att_vector.to(dml)
    ...
    def forward(
        ...
        **kwargs
        ...
        encoder_hidden = encoder_hidden.to(dml)
        src_mask = src_mask.to(dml)
        unroll_steps = unroll_steps.to(dml)
        hidden = hidden.to(dml)
        prev_att_vector = prev_att_vector.to(dml)
    ...
    def _init_hidden(self, encoder_final: Tensor = None) -> (Tensor, Optional[Tensor]):
        ...
        """
        encoder_final = encoder_final.to(dml)
