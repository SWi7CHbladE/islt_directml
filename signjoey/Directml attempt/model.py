...
class SignModel(nn.Module):
    ...
    def forward(
        ...
        sgn = sgn.to(dml)
        sgn_mask = sgn_mask.to(dml)
        sgn_lengths = sgn_lengths.to(dml)
        txt_input = txt_input.to(dml)
        txt_mask = txt_input.to(dml)
        """
    ...
    def decode(
        ...
        if(decoder_hidden != None):
            decoder_hidden = decoder_hidden.to(dml)
        if(sgn_mask != None):
            sgn_mask = sgn_mask.to(dml)
        if(txt_input != None):
            txt_input = txt_input.to(dml)
